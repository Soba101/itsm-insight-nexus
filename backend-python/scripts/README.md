...
│   ├── benchmark_similarity_search.py
│   ├── benchmark_e2e_pipeline.py
│   ├── analyze_similarity_distribution.py
│   └── evaluate_parent_child_links.py
├── embedding_worker.py        # Background embedding processor
├── establish_ticket_relationships.py  # Create parent-child links
└── populate_embeddings.py     # Batch embedding generation
```
...
All performance evaluation scripts are located in `scripts/performance_eval/`.
